...
class ICFFile{
    ...
    struct ICFBunchTrailer{
        ...
        ICFBunchTrailer(uint64_t file_offset,
        uint64_t prev_bunch_offset,
        uint64_t bunch_data_offset,
        uint32_t n_chunks_in_bunch,
        uint32_t bunch_number):file_offset_(file_offset),
                                prev_bunch_offset_(prev_bunch_offset),
                                bunch_data_offset_(bunch_data_offset),
                                n_chunks_in_bunch_(n_chunks_in_bunch),
                                bunch_number_(bunch_number),
                                version_(0){
            time_stamp2_ = std::chrono::system_clock::now();
            time_stamp_ = std::chrono::system_clock::to_time_t(time_stamp2_);
        }
...
public:
    ...
    ICFFile(std::string path, ICFFile::access_mode mode=ICFFile::append, uint32_t bunchsize = 1000000);
    ...
    void close(){flush();file_handle_.close(); }
...
private:
    ...
    std::vector<std::shared_ptr<std::vector<char> > > write_buffer_;
    uint32_t n_entries_;
    uint32_t cbunchoffset_;
    uint32_t bunchsize_;
    uint64_t last_bunch_fp_;
    uint32_t bunch_number_;
